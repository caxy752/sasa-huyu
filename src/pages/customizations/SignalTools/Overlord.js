...
const Overlord = () => {
    ...
    const getStoredAuthContext = useCallback(() => {
        try {
            ...
            console.error('[Overlord] Failed to parse auth context:', error);
            return null;
        }
    }, []);
    ...
    const requestProposal = useCallback(() => {
        ...
        logMessage(`📋 Sending proposal: ${derivContractType} on ${symbolRef.current} x${currentStakeRef.current}`);
    ...
    */
            ...
            if (data.msg_type === 'buy') {
                ...
                const displayType =
                    ...
                    getDerivContractType(activeContractTypeRef.current);
